...
features_pos = zeros(0,(feature_params.template_size / feature_params.hog_cell_size)^2 * 31);
for im = 1:num_images
    path = strcat(train_path_pos, '/', image_files(im).name);
    image =  single(imread(path));
    Hog1 = vl_hog(image,feature_params.hog_cell_size);
    Hog2 = vl_hog(flip(image,2),feature_params.hog_cell_size);
    features_pos(end+1,:) = Hog1(:);
    features_pos(end+1,:) = Hog2(:);
end
end
